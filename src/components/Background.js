...
const Background = () => {
  ...
  useEffect(() => {
    ...
    image.src = '/tree.png'; // make sure the image is in your public folder
    let raindrops = [];
    ...
    function drawLightning() {
        ...
        const radius = Math.random() * (window.innerWidth / 4) + (window.innerWidth / 4); // randomize the size a bit
        ...
        gradient.addColorStop(0, `rgba(138, 43, 226, ${Math.random() * 0.8 + 0.2})`); // inner color - stronger
        gradient.addColorStop(1, 'rgba(138, 43, 226, 0)'); // outer color - fully transparent
        ...
        lightningTimeout = setTimeout(() => {
          lightningCtx.clearRect(0, 0, window.innerWidth, window.innerHeight);
        }, 300 + Math.random() * 200); // hold the 'lightning' a bit before clearing
      }
      ...
      image.onload = () => {
        ...
        createRain();
        animate();
        setInterval(drawLightning, 5000); // adjust for more or less frequent lightning
      };
    }, []);
